  ...
  >
    ...
    >
      ...
      >
        ...
        <nuxt-link :key="item.name" v-for="item in items" :to="item.path">
          <li
            class="nav-item"
            :class="[
              { [currentClasses]: routeName === item.name },
              hoverClasses
            ]"
          >
            {{ item.title }}
...
export default class Navigation extends Vue {
  ...
  @Prop({ required: false, default: " hover:bg-black hover:bg-opacity-40" })
  hoverClasses!: string;
  @Prop({
    required: false,
  ...
  onShowChanged(val: boolean) {
    bus.$emit("navigationStateChanged", val);
  }

  mounted() {
    bus.$on("navigationStateChanged", (open: boolean) => {
      open && document.body.style.setProperty('overflow', 'hidden')
      open || document.body.style.removeProperty('overflow')
    });

    this.observer = new IntersectionObserver(
      ([entry]) => {
        this.sticking = !(entry as any).isVisible;
      },
      { threshold: 0.0 }
    );
